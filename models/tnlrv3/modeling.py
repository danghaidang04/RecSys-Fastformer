...
class TuringNLRv3Model(TuringNLRv3PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not isinstance(config, TuringNLRv3ForSeq2SeqConfig):
            ...
            self.pooler = None
        # Sua dong nay nhe, ban dau la: getattr(self.config, 'rel_pos_bins', 0) > 0:
        if getattr(self.config, 'rel_pos_bins', 0) > 0:
            self.rel_pos_bias = nn.Linear(getattr(self.config, 'rel_pos_bins', 0), config.num_attention_heads, bias=False)
        else:
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
        ...
        if getattr(self.config, 'rel_pos_bins', 0) > 0:
            ...
            rel_pos = relative_position_bucket(
                rel_pos_mat, num_buckets=getattr(self.config, 'rel_pos_bins', 0), max_distance=getattr(self.config, 'max_rel_pos', 0))
            rel_pos = F.one_hot(rel_pos, num_classes=getattr(self.config, 'rel_pos_bins', 0)).type_as(embedding_output)
